...
#include "TH2D.h"
#include <math.h>
//This root macro is for use with the histograms gated on the 238 keV gamma. 
	//Set Parameters here!
	int rebinning = 1;
	int ExStart = 40; //Excitation energy starting point (multiplied by 10);
	int ExEnd = 50;

	TString Histograms = "SiQQQGamGated2";
	TFile *Hists = TFile::Open("../" + Histograms + ".root");


	//Draw the hists we want to copy first!
	Hists->cd("Ex_vs_Gammas_Hists/Si_v_Gam/Si_DoubleGammaGated");
//Must change the names of these histograms
	Si_Gam2381297_Ex_40_50->Draw();
	Hists->cd("Ex_vs_Gammas_Hists/Si_v_Gam/Si_DoubleGammaGated_Bkg");
	Si_Gam2381297_Ex_40_50_Bkg->Draw();

  	TFile* fileResults = TFile::Open("Si_Gam2381297/Ex_" + TString(std::to_string(ExStart)) + "_" + TString(std::to_string(ExEnd)) + ".root","RECREATE");
   	fileResults->cd();

	//Get the signal histogram and copy it into the new file:
	TDirectory *dir1 = (TDirectory*)Hists->FindObject("Ex_vs_Gammas_Hists");
	TDirectory *dir2 = (TDirectory*)dir1->FindObject("Si_v_Gam");
	TDirectory *dir3 = (TDirectory*)dir2->FindObject("Si_DoubleGammaGated");
	TH1D *SigHist = (TH1D*)dir3->FindObject("Si_Gam2381297_Ex_" + TString(std::to_string(ExStart)) + "_" + TString(std::to_string(ExEnd)));

	//Get the background histogram and copy it into the new file:
	TDirectory *dir4 = (TDirectory*)dir2->FindObject("Si_DoubleGammaGated_Bkg");
	TH1D *BackHist = (TH1D*)dir4->FindObject("Si_Gam2381297_Ex_" + TString(std::to_string(ExStart)) + "_" + TString(std::to_string(ExEnd)) + "_Bkg");
...
//		RatioHist2->Draw("SAME");
	...
	ofstream outfile("Si_Gam2381297/Ex_" + std::to_string(ExStart) + "_" + std::to_string(ExEnd) + "_2.txt");
