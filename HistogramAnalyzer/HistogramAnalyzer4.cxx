...
//This root macro is for use with the histograms gated on the 238 keV gamma. 
	...
	Hists->cd("Ex_vs_Gammas_Hists/Si_v_Gam/Si_SingleGammaGated");
//Must change the names of these histograms
	Si_Gam238_Ex_40_50->Draw();
	Hists->cd("Ex_vs_Gammas_Hists/Si_v_Gam/Si_SingleGammaGated_Bkg");
	Si_Gam238_Ex_40_50_Bkg->Draw();

  	TFile* fileResults = TFile::Open("Si_Gam238/Ex_" + TString(std::to_string(ExStart)) + "_" + TString(std::to_string(ExEnd)) + ".root","RECREATE");
   	fileResults->cd();
	...
	TDirectory *dir3 = (TDirectory*)dir2->FindObject("Si_SingleGammaGated");
	TH1D *SigHist = (TH1D*)dir3->FindObject("Si_Gam238_Ex_" + TString(std::to_string(ExStart)) + "_" + TString(std::to_string(ExEnd)));

	//Get the background histogram and copy it into the new file:
	TDirectory *dir4 = (TDirectory*)dir2->FindObject("Si_SingleGammaGated_Bkg");
	TH1D *BackHist = (TH1D*)dir4->FindObject("Si_Gam238_Ex_" + TString(std::to_string(ExStart)) + "_" + TString(std::to_string(ExEnd)) + "_Bkg");
...
//		RatioHist2->Draw("SAME");
	...
	ofstream outfile("Si_Gam238/Ex_" + std::to_string(ExStart) + "_" + std::to_string(ExEnd) + "_2.txt");
